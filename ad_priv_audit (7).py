...
# Module-level logger. Configured by setup_logging() in main() based on flags.
log = logging.getLogger("ad_priv_audit")


class _ColoredFormatter(logging.Formatter):
    """Console formatter that adds severity-based prefixes for visual scanning."""
    LEVEL_PREFIX = {
        "DEBUG":   "  [debug]",
        "INFO":    "  [info] ",
        "WARNING": "  [WARN] ",
    ...
    def format(self, record):
        ...
        # If the message starts with our existing print-style markers, leave it alone
...
def setup_logging(verbose: bool = False, log_file: Optional[str] = None):
    """
    Initialize logging. Default level is WARNING (only warnings/errors visible).
    --verbose enables INFO and DEBUG. Optional --log-file writes everything
    at DEBUG level to a file regardless of console verbosity.
    """
    log.setLevel(logging.DEBUG)  # Logger captures everything; handlers filter
    log.handlers.clear()  # Avoid duplicate handlers if called twice
    ...
    # Optional file handler — always full debug
    if log_file:
        try:
...
def timed_check(label: str = None):
    """
    Decorator that logs entry, timing, and result count for detection functions.
    Produces warnings when a check returns empty/no results on expected-populated
    environments — helpful for spotting silent failures.

    Usage:
        @timed_check("Kerberoastable")
        def check_kerberoastable(...) -> list[Finding]: ...
    """
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                log.error(f"{name} raised {type(e).__name__} after {elapsed:.1f}s: {e}")
                raise
            elapsed = time.perf_counter() - t0
            ...
            if isinstance(result, list):
                count = len(result)
                if count == 0:
                    log.info(f"<< {name} completed in {elapsed:.1f}s: 0 results")
                else:
                    log.info(f"<< {name} completed in {elapsed:.1f}s: {count} result(s)")
            elif isinstance(result, tuple) and len(result) == 2 and isinstance(result[0], list):
                count = len(result[0])
                raw_size = len(result[1]) if hasattr(result[1], "__len__") else "?"
                log.info(f"<< {name} completed in {elapsed:.1f}s: {count} finding(s), raw data: {raw_size}")
            else:
...
def run_ps(command: str, timeout: int = 120) -> Optional[str]:
    # Log a short preview of the command at debug level
    preview = command.strip().replace("\n", " ").replace("\r", "")[:120]
    log.debug(f"PS cmd: {preview}{'...' if len(command) > 120 else ''}")
    try:
        ...
        if result.returncode != 0:
            log.warning(f"PowerShell exit {result.returncode}: {result.stderr.strip()[:300]}")
            log.debug(f"Full failing command: {command}")
            return None
        # Even on success, surface stderr content as a warning — PowerShell often
        # writes non-fatal errors to stderr while still exiting 0
        if result.stderr.strip():
            log.warning(f"PowerShell stderr (non-fatal): {result.stderr.strip()[:300]}")
        out = result.stdout.strip()
        log.debug(f"PS returned {len(out)} chars")
        return out
    except subprocess.TimeoutExpired:
        log.error(f"PowerShell command timed out after {timeout}s")
        log.debug(f"Timed-out command: {command[:200]}")
        return None
    except FileNotFoundError:
        log.error("PowerShell not found. Run this on a Windows workstation.")
        return None
...
def ps_json(command: str, timeout: int = 120) -> Optional[list | dict]:
    ...
    if raw is None:
        return None
    if not raw:
        log.debug("PS returned empty stdout (no data to parse)")
        return None
    try:
        data = json.loads(raw)
        if isinstance(data, dict):
            data = [data]
        log.debug(f"PS JSON parsed: {len(data)} item(s)")
        return data
    except json.JSONDecodeError as e:
        log.warning(f"JSON parse error: {e}. First 200 chars of output: {raw[:200]!r}")
        return None
...
# ============================================================================
# Group Member and User Caches
# ============================================================================

_member_cache: dict[str, Optional[list[dict]]] = {}
_user_cache: dict[str, Optional[dict]] = {}
_objclass_cache: dict[str, Optional[str]] = {}
_cache_hits = 0
_cache_misses = 0
_user_cache_hits = 0
...
def _ps_quote(s: str) -> str:
    """Escape a string for safe embedding in a single-quoted PowerShell literal."""
    return s.replace("'", "''")


def get_user_enrichment(sam: str) -> Optional[dict]:
    """
    Fetch enrichment properties for a single user/computer account, with caching.

    Returns dict of properties or None if the account couldn't be resolved.
    Each unique SAM is queried at most once across the entire run.
    """
    ...
    if cache_key in _user_cache:
        _user_cache_hits += 1
        log.debug(f"User cache hit: {sam}")
        return _user_cache[cache_key]
    _user_cache_misses += 1
    log.debug(f"User cache miss, querying: {sam}")

    sam_safe = _ps_quote(sam)
    cmd = f"""
    try {{
        $u = Get-ADUser -Identity '{sam_safe}' -Properties `
            Enabled, LastLogonDate, PasswordLastSet, PasswordNeverExpires, `
            AdminCount, Description, whenCreated, MemberOf, `
            DisplayName, SmartcardLogonRequired -ErrorAction Stop
        [PSCustomObject]@{{
            Enabled = $u.Enabled
            ...
            SmartcardLogonRequired = $u.SmartcardLogonRequired
    ...
    """
    result = ps_json(cmd, timeout=30)
    if result:
        data = result[0] if isinstance(result, list) else result
        _user_cache[cache_key] = data
        return data
    log.warning(f"User enrichment returned no data for '{sam}' (account may not exist as user, "
                f"may be a contact/foreign principal, or query failed)")
    _user_cache[cache_key] = None
    return None


def resolve_object_class(sam: str) -> Optional[str]:
    """
    Resolve a SAM account name to its objectClass (user, group, computer, etc.)
    via Get-ADObject. Cached. Returns None if not resolvable.
    """
    if not sam:
        return None
    cache_key = sam.lower()
    if cache_key in _objclass_cache:
        log.debug(f"ObjClass cache hit: {sam}")
        return _objclass_cache[cache_key]
    log.debug(f"ObjClass cache miss, resolving: {sam}")

    sam_safe = _ps_quote(sam)
    cmd = f"""
    try {{
        $o = Get-ADObject -Filter "SAMAccountName -eq '{sam_safe}'" -Properties objectClass -ErrorAction Stop
        if ($o) {{
            [PSCustomObject]@{{
    ...
    if result:
        data = result[0] if isinstance(result, list) else result
        _objclass_cache[cache_key] = data
        return data
    log.debug(f"Could not resolve object class for SAM '{sam}' (may be SID-only or stale reference)")
    _objclass_cache[cache_key] = None
    return None


def get_group_direct_members(group_name: str) -> Optional[list[dict]]:
    """
    Get direct members of a group with per-user enrichment and caching.

    Two-phase approach:
      1. Enumerate group members via Get-ADGroupMember (minimal call — most reliable)
      2. For each user/computer, fetch enrichment via cached get_user_enrichment()

    This isolates failures: one bad user enrichment doesn't break the whole group's
    output. Each unique user is queried at most once across the entire run.
    """
    global _cache_hits, _cache_misses
    cache_key = group_name.lower()
    if cache_key in _member_cache:
        _cache_hits += 1
        log.debug(f"Group cache hit: {group_name}")
        return _member_cache[cache_key]
    _cache_misses += 1
    log.debug(f"Group cache miss, enumerating: {group_name}")

    # Phase 1: minimal group member enumeration
    group_safe = _ps_quote(group_name)
    cmd_members = f"""
    Get-ADGroupMember -Identity '{group_safe}' |
    Select Name, SamAccountName, distinguishedName, objectClass |
    ConvertTo-Json -Depth 2
    """
    members = ps_json(cmd_members)
    if not members:
        log.warning(f"Group '{group_name}' returned zero members. "
                    f"Possible causes: group does not exist, no members, RPC/ADWS issue, "
                    f"or insufficient permissions to read membership.")
        _member_cache[cache_key] = []
        return []

    log.info(f"Group '{group_name}': {len(members)} direct member(s)")

    # Phase 2: enrich each non-group member individually
    enriched = []
    enrich_failures = 0
    for m in members:
        obj_class = (m.get("objectClass") or "").lower()
        sam = m.get("SamAccountName", "")
        record = dict(m)

        if obj_class == "group" or not sam:
            enriched.append(record)
            continue

        enrichment = get_user_enrichment(sam)
        if enrichment:
            record.update(enrichment)
        else:
            enrich_failures += 1
            log.warning(f"Could not enrich {sam} in '{group_name}' (continuing with base data)")

        enriched.append(record)

    if enrich_failures > 0:
        log.warning(f"Group '{group_name}': {enrich_failures}/{len(members)} member enrichment(s) failed")

    _member_cache[cache_key] = enriched
    return enriched
...
def get_user_cache_stats() -> tuple[int, int]:
    return _user_cache_hits, _user_cache_misses
...
@timed_check("AdminCount Discovery")
def discover_admincount_groups() -> list[DiscoveryResult]:
    ...
    results = ps_json(cmd)
    if not results:
        log.warning("AdminCount discovery returned no groups. AD has at least the built-in "
                    "Tier 0 groups with AdminCount=1; if this is empty, the query failed silently "
                    "or the source account lacks read permission.")
        return []
    log.info(f"AdminCount discovery: {len(results)} group(s) found")
    discovered = []
...
@timed_check("Name Pattern Discovery")
...
@timed_check("Dangerous ACL Discovery")
def discover_dangerous_acls() -> list[DiscoveryResult]:
    ...
    (Get-Acl "AD:\\$domain").Access |
        ...
        Select @{{N='Identity';E={{$_.IdentityReference.Value}}}},
               @{{N='Right';E={{ if ($_.ObjectType -eq '{GUID_REPL_CHANGES}') {{ 'Repl-Changes' }} else {{ 'Repl-Changes-All' }} }}}} |
        ConvertTo-Json -Depth 2
    """
    ...
    if dcsync_results:
        for r in dcsync_results:
            ...
            # Resolve principal via cached helper — only emit groups
            obj = resolve_object_class(name)
            if obj and (obj.get("ObjectClass") or "").lower() == "group":
                discovered.append(DiscoveryResult(
                    group_name=name,
                    distinguished_name=obj.get("DistinguishedName", ""),
                    discovery_method="acl_dcsync",
                    detail=f"Has {r.get('Right', 'DCSync')} rights on domain root - can extract password hashes",
                    already_known=name.lower() in defaults_lower
                ))

    # ----- 3b: Write access to Tier 0 groups -----
    print("    Checking write access to Tier 0 groups...")
    for target_group in ["Domain Admins", "Enterprise Admins", "Administrators"]:
        target_safe = _ps_quote(target_group)
        acl_cmd = f"""
        try {{
            $dn = (Get-ADGroup '{target_safe}').DistinguishedName
            ...
            for r in results:
                ...
                if obj and (obj.get("ObjectClass") or "").lower() == "group":
                    discovered.append(DiscoveryResult(
                        ...
                        detail=f"Has {r.get('Rights', 'write')} on {target_group} - can modify membership",
                        already_known=name.lower() in defaults_lower
                    ))

    # ----- 3c: GPO modification rights on DC OU -----
    ...
    # ----- 3d: Dangerous rights on domain root -----
...
def build_group_tree(group_name: str, depth: int = 0, parent_path: list = None,
    ...
    if group_name.lower() in visited:
        log.warning(f"Circular nesting detected: {group_name} (skipping)")
        return None
...
@timed_check("Unconstrained Delegation")
...
@timed_check("Constrained Delegation w/ Protocol Transition")
...
@timed_check("Resource-Based Constrained Delegation")
...
@timed_check("Password Not Required")
...
@timed_check("Pre-Win2K Compat Access")
...
@timed_check("Reversible Password Encryption")
...
@timed_check("Inactive Computers")
...
@timed_check("ADCS Enrollment Services")
...
@timed_check("Foreign Security Principals")
...
@timed_check("Local Admin Enumeration")
    ...
    def enum_one(comp):
        ...
        try:
            ...
            if "unreachable" in err or "timed out" in err or "refused" in err:
                log.debug(f"Local admin enum unreachable: {host}")
                return host, None, "unreachable"
            log.debug(f"Local admin enum error on {host}: {type(e).__name__}: {e}")
...
@timed_check("Session Enumeration")
    ...
    def enum_one(comp):
        ...
        try:
            ...
            if "unreachable" in err or "timed out" in err or "refused" in err or "access_denied" in err:
                log.debug(f"Session enum unreachable/denied: {host}")
                return host, None, "unreachable"
            log.debug(f"Session enum error on {host}: {type(e).__name__}: {e}")
            return host, None, "error"
...
def correlate_findings(findings, trees, acl_data=None, local_admins=None,
    ...
    log.info(f"Correlation inputs: {len(kerberoastable)} kerberoastable, "
             f"{len(asrep_roastable)} AS-REP, {len(adcs_esc1)} ESC1, "
             f"{len(unconstrained_computers)} unconstrained-delegation hosts, "
             f"{len(password_not_req)} password-not-required, "
             f"{sum(len(by_cat[c]) for c in acl_abuse_cats)} ACL-abuse total")
    if not (acl_data or local_admins or session_data):
        log.info("Correlation running on default-tier data only (no ACL/local-admin/session data collected)")
...
def main():
    ...
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="Enable verbose (INFO + DEBUG) console output")
    parser.add_argument("--log-file", type=str, default=None,
                        help="Write full debug log to specified file")
    args = parser.parse_args()

    setup_logging(verbose=args.verbose, log_file=args.log_file)
    log.debug(f"Invoked with args: {vars(args)}")
    ...
    if args.demo:
        ...
        if not check:
            log.error("ActiveDirectory PowerShell module not found.")
            log.error("Install with: Install-WindowsFeature RSAT-AD-PowerShell")
            log.error("Or run with --demo to test the report format.")
            sys.exit(1)
        ...
        hits, misses = get_cache_stats()
        u_hits, u_misses = get_user_cache_stats()
        print(f"\n  [Cache] Group: {hits} hits / {misses} PS queries  |  User: {u_hits} hits / {u_misses} PS queries")
